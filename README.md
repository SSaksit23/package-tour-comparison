# 📊 Travel Itinerary Intelligence Platform

An advanced AI-powered system for in-depth travel itinerary comparison, analysis, and strategic planning. The platform leverages a sophisticated **Hybrid Retrieval-Augmented Generation (RAG)** architecture, enriched with real-time web data and powered by a collaborative multi-agent system.
...
| 💬 **Q&A Chat** | Ask questions about your documents with RAG-enhanced answers |
...
The platform utilizes a modular, service-oriented architecture that ensures scalability, maintainability, and flexibility.

| Component | Technology Stack | Core Responsibility |
|-----------|------------------|---------------------|
| **Frontend** | React, Vite, TypeScript | Provides a responsive and intuitive user interface for file management, analysis visualization, and results exploration |
| **Backend Services** | Python (FastAPI), Node.js | Manages API requests, orchestrates the data processing workflow, and coordinates the various AI and data services |
| **AI Orchestration** | LangChain, CrewAI | Manages the logic for data processing chains, vectorization, and the coordination of the multi-agent system |
| **Vector Database** | ChromaDB, ArangoDB | Stores high-dimensional vector embeddings of text chunks for efficient and scalable similarity search |
| **Knowledge Graph** | ArangoDB | Models itineraries, destinations, attractions, and their relationships as a graph for deep contextual and relational queries |
| **PDF Extraction** | PyMuPDF (fitz) | Reliable PDF text extraction with superior handling of complex layouts, Thai text, and structured data |
...
### 3. Hybrid RAG and Knowledge Enrichment

This stage builds a comprehensive and interconnected knowledge base:

1. **Vectorization**: Each text chunk is converted into a numerical vector embedding using OpenAI's `text-embedding-3-small`
2. **Vector Storage**: Embeddings are stored in ChromaDB/ArangoDB for fast similarity searches
3. **Knowledge Graph Construction**: LLM analyzes text to identify key entities (Destination, Attraction, Price, Flight) and their relationships, populating ArangoDB graph
4. **Web-Enabled Data Enrichment**: Web Search Agent autonomously searches the web for supplementary data (reviews, news, competitor offerings) and feeds it back into the RAG system
...
│          ↓                                               │
│     ┌──────────────┐                                    │
│     │ Web Search   │ → Enrich with real-time data       │
│     │ Agent (EXA)  │                                    │
│     └──────────────┘                                    │
│                                                          │
...
The platform leverages a collaborative multi-agent system, orchestrated by CrewAI, to perform deep and insightful analysis:

#### The Agentic Workforce

**Travel Analyst Crew** (Multi-Agent System):
- **Document Analyst**: Extracts and structures key information from travel itineraries
- **Market Researcher**: Researches current market prices, competitor offerings, and industry trends
- **Strategic Advisor**: Generates insights and recommendations

**Smart Product Launch Agent**:
- Analyzes tour market positioning
- Identifies unique selling propositions (USPs)
- Evaluates competitive advantages
- Assesses products against market data from web search

**AI Consultant Agent** (with Persistent Memory):
- Synthesizes information across multiple analyses
- Identifies long-term trends using historical context
- Provides high-level recommendations for:
  - Product optimization
  - Market expansion
  - Risk mitigation
- Leverages Mem0 for persistent memory of past analyses

#### Report Synthesis

The collective insights from the agentic workforce are compiled into comprehensive, human-readable reports that provide:
- Strategic analysis (not just comparison)
